...
class IsUserAdminMiddleware
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        /* try login user */
        $result = NvoqNetworkController::GenAI_IsUserAdmin($request);

        if(!$result)
        {
            Log::debug('[GenAI API ADMIN AUTH] failed to login to nvoq servers or user is not an administrator');
            return
                new JsonResponse(['error'=>'Unauthorized'],
                    \Symfony\Component\HttpFoundation\Response::HTTP_UNAUTHORIZED);
        }

        Log::debug('[GenAI API ADMIN AUTH] logged in to nvoq successfully | user: ' . session("username") . ' and user is admin');
        return $next($request);
    }
}
